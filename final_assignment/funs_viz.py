...
def tidy_results(results,formulation,planning_steps):
    ...
    if formulation == 6 and planning_steps == 3:
    ...
    elif formulation == 3 and planning_steps== 3:
        experiments, outcomes = results
    ...
    elif formulation == 6 and planning_steps == 1:
        experiments, outcomes = results
        ...
        melted_df=pd.melt(results_df, id_vars=['A.0_ID flood wave shape', 'discount rate 0','EWS_DaysToThreat', 'scenario', 'policy', 'model', 'RfR Total Costs', 'Expected Evacuation Costs'], 
                        value_vars=cols_to_melt["Bmax"],
                        value_name="Bmax", 
                        var_name="Dike ring"
            )
