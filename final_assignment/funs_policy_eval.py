...
def create_collaborative_policies(file_name, dike_model):
    # Read the CSV file into a DataFrame
    df = pd.read_csv("policies/" + str(file_name))
    ...
    for index, row in df.iterrows():
        # Extract the values from the row
        name_of_policy = df["Policy code"][index]
        rfr_values = [row[f"{i}_RfR 0"] for i in range(5)]
        dike_increase_values = [row[f"A.{i}_DikeIncrease 0"] for i in range(1, 6)]
        ews_days_to_threat = row['EWS_DaysToThreat']
        ...
        # Create a dictionary for the policy
        policy_dict = {
            '0_RfR 0': int(rfr_values[0]),
            '1_RfR 0': int(rfr_values[1]),
            '2_RfR 0': int(rfr_values[2]),
            '3_RfR 0': int(rfr_values[3]),
            '4_RfR 0': int(rfr_values[4]),
            'A.1_DikeIncrease 0': int(dike_increase_values[0]),
            'A.2_DikeIncrease 0': int(dike_increase_values[1]),
            'A.3_DikeIncrease 0': int(dike_increase_values[2]),
            'A.4_DikeIncrease 0': int(dike_increase_values[3]),
            'A.5_DikeIncrease 0': int(dike_increase_values[4]),
            'EWS_DaysToThreat': int(ews_days_to_threat)
        }
